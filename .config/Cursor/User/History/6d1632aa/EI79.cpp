...
{
    auto window = g_pHyprOpenGL->m_pCurrentWindowData.lock();
    bool shouldInvert =
...
void WindowInverter::OnRenderWindowPost()
{
    if (m_ShadersSwapped)
    {
        ...
        {
            for (auto& decoration : g_pHyprOpenGL->m_pCurrentWindowData.lock()->m_dWindowDecorations)
            {
                // Debug::log(LOG, "REMOVE: Window {:p}, Decoration {:p}", (void*)g_pHyprOpenGL->m_pCurrentWindowData.get(), (void*)decoration.get());
...
void WindowInverter::InvertIfMatches(PHLWINDOW window)
{
    ...
    if (shouldInvert != (windowIt != m_InvertedWindows.end()))
    {
        ...
        PHYPRRENDERER->damageWindow(window);
    }
}
