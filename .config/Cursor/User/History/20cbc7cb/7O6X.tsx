...
export const MultiStepFormContextProvider = ({
  ...
  const {
    ...
    watch,
    setValue
  } = useForm({ defaultValues: getAllLocalStorageValues() });
  ...
  const submitForm = async () => {
    const res = await invoke<GenericPostResponse>("create_personal", {
      personal: getAllLocalStorageValues()
    });
  ...
  const submitHandler = handleSubmit((data) => {
    ...
    fields.forEach((field) => {
      localStorage.setItem(field, allFields[field]);
    });

    navigate({
      to: `/personal/personal/file/${steps[currentStep + 1]}/`,
      viewTransition: true,
      resetScroll: true
    });
  });
