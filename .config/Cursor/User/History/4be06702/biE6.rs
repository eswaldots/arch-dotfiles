...
pub fn date_to_sql(date: &str) -> sqlx::types::time::PrimitiveDateTime {
    ...
    if date.contains('T') {
        ...
        if parts.len() > 1 {
            ...
            let sql_date = Date::from_calendar_date(year, month.try_into().unwrap_or(Month::January), day).unwrap_or_else(|_| Date::from_calendar_date(2000, Month::January, 1).unwrap());
    ...
    // Si hay algún problema, devolvemos una fecha predeterminada
    let default_date = Date::from_calendar_date(2000, Month::January, 1).unwrap();
    let default_time = Time::from_hms(0, 0, 0).unwrap();
    PrimitiveDateTime::new(default_date, default_time)
}