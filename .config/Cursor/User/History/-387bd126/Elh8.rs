...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // store.set("personal_local_mode", true);

            block_on(async move {
                if let Err(e) = setup::initialize(app).await {
                    match e {
                        SetupError::DatabaseError(e) => {
                            println!("Error conecting to db, initializing local database: {}", e);
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // ANALYSIS COMMANDS
            commands::analysis_commands::save_repein,
            commands::analysis_commands::get_public_repein,
            commands::analysis_commands::get_repein_records_by_id,
            commands::analysis_commands::save_bounding,
            commands::analysis_commands::get_public_boundings,
            commands::analysis_commands::get_public_repein_by_id,
        ])
