...
static auto* const PHYPRRENDERER = &HyprRenderer::CInstance::get();
inline CConfigManager* g_pConfigManager = nullptr;
inline CHyprOpenGLImpl* g_pHyprOpenGL = nullptr;

inline PHL_PLUGIN_HANDLE PHANDLE = nullptr;  // Changed from HANDLE

inline WindowInverter g_WindowInverter;
inline std::mutex g_InverterMutex;

inline std::vector<SP<HOOK_CALLBACK_FN>> g_Callbacks;  // Changed from CFunctionHook
HOOK_CALLBACK_FN* g_getDataForHook;  // Changed from CFunctionHook
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(PHL_PLUGIN_HANDLE handle)
{
    PHANDLE = handle;
