...
pub async fn initialize_local_database(app: &AppHandle) -> Result<(), sqlx::Error> {
    ...
    check_database(&app, &Database::Sqlite(pool.clone())).await?;
    
    // Create new state instead of trying to access existing one
    let new_state = Mutex::new(AppState {
        db: Database::Sqlite(pool.clone())
    });
    
    // Manage the state directly on the app handle
