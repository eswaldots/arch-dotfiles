...
pub async fn initialize_database(app: &mut App) -> Result<Database, DatabaseError> {
    ...
    // Manage the state here, ensuring it's done before any commands run
    app.manage(Mutex::new(AppState { db: db.clone() }));
    
    Ok(db)
}

// This function is now simplified since state management happens in initialize_database
pub async fn initialize_local_database(app: &AppHandle) -> Result<(), sqlx::Error> {
    ...
    // No state management here - it should be done in the main initialization
    Ok(())
}
