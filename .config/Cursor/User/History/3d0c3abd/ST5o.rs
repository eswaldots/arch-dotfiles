...
pub async fn initialize_local_database(app: &AppHandle) -> Result<(), sqlx::Error> {
    ...
    // Intentamos obtener el estado actual para verificar si ya está gestionado
    match app.try_state::<Mutex<AppState>>() {
        Some(_) => {
            // El estado ya existe, no podemos sobrescribirlo directamente
            // Podríamos implementar un mecanismo para notificar al frontend
            // que necesita reiniciar la aplicación
            println!("Estado ya gestionado, no se puede sobrescribir. Se requiere reinicio.");
        },
        None => {
            // El estado no existe, lo gestionamos
