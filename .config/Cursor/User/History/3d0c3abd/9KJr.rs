...
pub async fn initialize_database(app: &mut App) -> Result<Database, DatabaseError> {
    ...
    match local_mode {
        Some(true) => {
            ...
            // Gestionar el estado aquí para asegurar consistencia
            app.manage(Mutex::new(AppState { db: Database::Sqlite(pool.clone()) }));
        ...
        Some(false) => {
            ...
            match PgPool::connect(&db_direction).await {
                Ok(pool) => {
                    check_database(&app.app_handle(), &Database::PostgreSQL(pool.clone())).await?;
                    
                    // Gestionar el estado aquí para asegurar consistencia
                    app.manage(Mutex::new(AppState { db: Database::PostgreSQL(pool.clone()) }));
                    
                    Ok(Database::PostgreSQL(pool))
                },
                Err(e) => {
                    println!("Error connecting to PostgreSQL: {}, falling back to SQLite", e);
                    
                    // Llamar a initialize_local_database en caso de error
                    initialize_local_database(app).await?;
                    
                    // Obtener el pool de la base de datos local
                    let pool = SqlitePool::connect("sqlite::memory:").await?;
                    Ok(Database::Sqlite(pool))
                }
            }
...
pub async fn initialize_local_database(app: &mut App) -> Result<(), sqlx::Error> {
    println!("Initializing local database");
    ...
    // Gestionar el estado aquí
    app.manage(Mutex::new(AppState { db }));
    
    println!("Local database initialized and state managed");
