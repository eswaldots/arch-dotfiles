...
import { getAllLocalStorageValues } from "@/services/local-storage.service";
...
function RouteComponent() {
  ...
  const formData = useMemo(() => {
    return getAllLocalStorageValues("repein_");
  }, []);
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      console.log(error)
      setRes(error as GenericPostResponse)
    }
  }, [submitForm]);
  ...
  const status = Boolean(res?.success);
