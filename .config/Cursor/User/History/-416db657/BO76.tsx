...
function RouteComponent() {
  const { bounding_res } = Route.useLoaderData();
  ...
  return (
    <section className="text-left w-screen px-8 flex flex-col gap-4">
      ...
      <ItemsTable>
        {bounding_res?.data?.map((band, index: number) => (
<Modal>
  <ModalContent>
    <StatusForm />
</ModalContent>
<ModalTrigger>
          ...
          >
            <div className="flex flex-col">
              <span className="font-semibold">N° {band.bounding_id} - {band.band_name} - {band.repein_ci} {band.repein_name} {band.repein_surnames} ({band.repein_nicknames})</span>
            </div>
          </ItemTable>
</ModalTrigger>
          </Modal>
