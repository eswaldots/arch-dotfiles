            ...
            <DropdownMenu>
              {routes.map((route, index) => (
                <Link
                  ...
                  to={route.path}
                  className={
                    "flex relative items-center border-transparent border px-2 py-2 rounded-md transition gap-2 hover:bg-neutral-900 hover:border-neutral-800"
                  }
                >
                  {route.path === actualRoute.pathname && <span className="absolute top-0 left-0 w-full h-full bg-primary rounded-md opacity-80"></span>}
                  <div className="size-6 opacity-60">{route.icon}</div>
