...
function RouteComponent() {
  ...
  const handleSelect = useCallback((member: PublicRepein) => {
    ...
    navigate({ to: "../bounding-band", from: Route.fullPath, viewTransition: true});
  }, [setMember, navigate]);
        ...
        {members?.map((member, index) => (
          ...
          >
            <div className="flex flex-col">
              ...
              <span className="text-sm text-neutral-100 font-medium">{member.repein_name}</span> • {{
                male: "Hombre",
                female: "Mujer",
              }[member.repein_gender] || "Genero desconocido"} • {{
                married: "Casado",
                single: "Soltero",
                divorced: "Divorciado",
                widowed: "Viudo"
              }[member.repein_state_civil] || "Estado civil desconocido"} • {member.repein_age > 1 ? "Años" : "Edad desconocida"}
              </span>
            </div>
          </ItemTable>
        ))}
