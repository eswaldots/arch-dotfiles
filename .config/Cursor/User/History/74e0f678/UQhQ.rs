    ...
    match pool {
        Database::Sqlite(pool) => {
            let query = if let Some(term) = search_term {
                sqlx::query("SELECT * FROM ? WHERE CAST(id AS TEXT) LIKE ? OR name LIKE ? OR description LIKE ?")
                    .bind(table_name)
                    .bind(format!("%{}%", term))
                    .bind(format!("%{}%", term)) 
                    .bind(format!("%{}%", term))
            } else {
                sqlx::query("SELECT * FROM ?").bind(table_name)
            }
                .bind(table_name)
                .fetch_all(pool)
                .await?;
        ...
        Database::PostgreSQL(pool) => {
            ...
            let query = sqlx::query(&query_str).fetch_all(pool).await?;
