...
pub async fn change_database_pool(
   ...
   match services::database_service::change_database_pool(&mut app_handle, module).await {
    Ok(_) => Ok(GenericPost {
        success: true,
        error: None,
    }),
    Err(err) => {
        ...
        services::database_service::initialize_local_database(&mut app_handle).await;
