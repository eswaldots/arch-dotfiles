...
#[tauri::command]
pub async fn get_boundings(state: State<'_, Mutex<AppState>>, search: Option<String>) -> CommandResult<Vec<Bounding>> {
    let db = &state.lock().await.db;

    let result = services::analysis_service::get_boundings(db, search).await;
