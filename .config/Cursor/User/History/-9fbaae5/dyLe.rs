use crate::{definitions::{analysis_definitions::{PublicRepein, Records, RepeinJSON}, app_definitions::AppState, responses_model::{to_command_result, to_status_result, CommandResult, StatusResult}}, services};
...
#[tauri::command]
pub async fn save_bounding(state: State<'_, Mutex<AppState>>, bounding_request: BoundingRequest) -> StatusResult {
    let db = &state.lock().await.db;
