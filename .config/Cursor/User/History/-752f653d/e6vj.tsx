...
export default function BoundingBandForm() {
  ...
  const onSubmit = (data: BoundingBandModel) => {
    setBoundingBand({
      bounding_type: data.bounding_type,
      document_id: data.document_id,
      date: data.date,
      organization_id: data.organization_id,
      reason: data.reason,
      has_procedure: data.has_procedure ?? false,
      procedure_description: data.procedure_description ?? "descripción del procedimiento",
    })
  ...
  return (
    <section className="text-left px-8 flex flex-col gap-4 w-[780px] bg-neutral-950 max-h-[100vh] overflow-y-auto py-12">
      <h1 className="text-2xl text-left font-semibold mb-4">
        Agregar Documento a Banda
      </h1>
