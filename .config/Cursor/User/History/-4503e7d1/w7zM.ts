...
const useBounding = create<BoundingStore>((set) => ({
  ...
  setMember: (member: PublicRepein) => set((state) => {
    if (state.member?.repein_id !== member.repein_id && 
        ((state.boundingMember?.length && state.boundingMember.length > 0) || 
         (state.boundingBand?.length && state.boundingBand.length > 0))) {
      return { 
        member,
        boundingBand: [],
        boundingMember: [] 
