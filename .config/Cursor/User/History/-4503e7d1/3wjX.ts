...
const useBounding = create<BoundingStore>((set, get) => ({
  member: null,
  band: null,
  boundingBand: [],
  boundingPerson: null,
  setBoundingBand: (data: BoundingBandModel) => set(() => {
    const boundingBand = get().boundingBand || [];
    return { boundingBand: [...boundingBand, data] };
  }),
