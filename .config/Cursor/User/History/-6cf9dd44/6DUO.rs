...
pub async fn get_public_repein(pool: &Database) -> Result<PublicRepein, sqlx::Error> {
    println!("Getting public repein");
...
match pool {
    Database::PostgreSQL(pool) => {
        ...
        let rows = sqlx::query(search_query).bind(search).fetch_one(pool).await?;

        println!("Public repein getted");

        let repein = PublicRepein {
            repein_id: rows.get("repein_id"),
