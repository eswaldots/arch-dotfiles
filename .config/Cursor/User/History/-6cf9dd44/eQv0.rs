...
pub async fn get_public_boundings(pool: &Database, search: Option<String>) -> Result<Vec<PublicBounding>, sqlx::Error> {
    let query = r"SELECT b.bounding_id AS bounding_id, b.repein_id AS repein_id, b.band_id AS band_id, b.status AS status, CAST(b.created_at AS TEXT) AS created_at, 
                  r.repein_ci AS repein_ci, r.repein_name AS repein_name, r.repein_surnames AS repein_surnames, r.repein_nicknames AS repein_nicknames, 
                  bandas.Nombre AS band_name 
                  FROM bounding b 
                  JOIN repein r ON bounding.repein_id = repein.repein_id 
                  JOIN bandas bandas ON bounding.band_id = bandas.id";
