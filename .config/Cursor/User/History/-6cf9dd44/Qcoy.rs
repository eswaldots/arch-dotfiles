use crate::{
    definitions::{analysis_definitions::{Bounding, BoundingRequest, Children, PublicRepein, Records, Relative, RepeinJSON}, database_definitions::Database}, utils::{date_to_sql::date_to_sql, generate_random_id::generate_random_id, string_to_bool::string_to_bool, string_to_int::string_to_int},
};
use sqlx::Row;
...
pub async fn get_boundings(pool: &Database, search: Option<String>) -> Result<Vec<Bounding>, sqlx::Error> {
    let query = r"SELECT bounding_id, repein_id, band_id, status, CAST(created_at AS TEXT) AS created_at FROM bounding";

    let search_query = r"SELECT bounding_id, repein_id, band_id, status, CAST(created_at AS TEXT) AS created_at FROM bounding WHERE bounding_id ILIKE '%' || $1 || '%' OR repein_id ILIKE '%' || $1 || '%' OR band_id ILIKE '%' || $1 || '%' OR status ILIKE '%' || $1 || '%' OR created_at ILIKE '%' || $1 || '%'";
    
    match pool {
        Database::PostgreSQL(pool) => {
            let mut boundings: Vec<Bounding> = Vec::new();
