...
pub async fn get_public_boundings(pool: &Database, search: Option<String>) -> Result<Vec<Bounding>, sqlx::Error> {
    let query = r"SELECT b.bounding_id, b.repein_id, b.band_id, b.status, CAST(b.created_at AS TEXT) AS created_at, 
                  r.repein_ci, r.repein_name, r.repein_surnames, r.repein_nicknames, 
                  bd.Nombre as band_name 
                  FROM bounding b 
                  JOIN repein r ON b.repein_id = r.repein_id 
                  JOIN bandas bd ON b.band_id = bd.id";
