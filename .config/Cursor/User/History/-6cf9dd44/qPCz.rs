...
pub async fn get_public_repein_by_id(pool: &Database, repein_id: i32) -> Result<PublicRepein, sqlx::Error> {
    println!("Getting public repein by id");

    let query = r"SELECT repein_id, personal_ci, personal_age, personal_state_civil, personal_age, personal_gender, personal_name, personal_surnames, personal_nicknames FROM repein WHERE repein_id = $1";

    match pool {
        Database::PostgreSQL(pool) => {
            let row = sqlx::query(query).bind(repein_id).fetch_one(pool).await?;

            let repein = PublicRepein {
                repein_id: row.get("repein_id"),
                repein_ci: row.get("personal_ci"),
                repein_age: row.get("personal_age"),
                repein_state_civil: row.get("personal_state_civil"),
                repein_gender: row.get("personal_gender"),
