...
pub async fn get_public_boundings(pool: &Database, search: Option<String>) -> Result<Vec<PublicBounding>, sqlx::Error> {
    let query = r"SELECT bounding.bounding_id, bounding.repein_id, bounding.band_id, bounding.status, CAST(bounding.created_at AS TEXT) AS created_at, 
                  repein.repein_ci, repein.repein_name, repein.repein_surnames, repein.repein_nicknames, 
                  bandas.Nombre as band_name 
                  FROM bounding 
                  JOIN repein ON bounding.repein_id = repein.repein_id 
                  JOIN bandas ON bounding.band_id = bandas.id";
